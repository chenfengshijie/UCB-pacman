...
class food_candidatesProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        """
        Stores the walls, pacman's starting position and food_candidates.
        """
        self.walls = startingGameState.getWalls()
        self.startingPosition = startingGameState.getPacmanPosition()
        top, right = self.walls.height-2, self.walls.width-2
        self.food_candidates = ((1, 1), (1, top), (right, 1), (right, top))
        for corner in self.food_candidates:
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        visited_cornor = state[1]
        return len(visited_cornor) == 4
    ...
    def getNextState(self, state, action):
        ...
        if next_node in self.food_candidates and next_node not in visited_cornors:
            visited_cornors.append(next_node)
        # you will need to replace the None part of the following tuple.
        return ((nextx, nexty), visited_cornors)
...
def food_candidatesHeuristic(state, problem):
    """
    A heuristic for the food_candidatesProblem that you defined.

      state:   The current search state
               (a data structure you chose in your search problem)

      problem: The food_candidatesProblem instance for this layout.

    This function should always return a number that is a lower bound on the
    shortest path from the state to a goal of the problem; i.e.  it should be
    admissible (as well as consistent).
    """
    food_candidates = problem.food_candidates  # These are the corner coordinates
    ...
    return minmanhattan(food_candidates, state[0])


class AStarfood_candidatesAgent(SearchAgent):
    "A SearchAgent for FoodSearchProblem using A* and your foodHeuristic"

    def __init__(self):
        self.searchFunction = lambda prob: search.aStarSearch(
            prob, food_candidatesHeuristic)
        self.searchType = food_candidatesProblem
